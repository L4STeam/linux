...
/* Internal alpha/beta are in units of 64ns to enable to use their full range
 * of values.
 */
...
{
	return ((u64)param * MAX_PROB >> ALPHA_BETA_SCALING) / NSEC_PER_SEC;
}

 static inline u32 dualpi2_unscale_alpha_beta(u32 param)
{
	return ((u64)param * NSEC_PER_SEC << ALPHA_BETA_SCALING) / MAX_PROB;
}
